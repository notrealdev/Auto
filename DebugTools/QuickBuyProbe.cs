...
// Gửi lệnh mua nhanh thuốc (lệnh native 327) vào ĐÚNG account đang chọn rồi đọc lại túi để biết server có bán thật không.
//
...
public static class QuickBuyProbe {
	public const string BuildStamp = "QUICK-BUY-20260925-01";
	// Phải khớp NativeBuildStamp trong SystemUint.cpp. Lệch nghĩa là tiến trình game còn giữ DLL cũ, chưa có lệnh 327.
	private const ulong ExpectedNativeBuildStamp = 99990010;
	private const int PollIntervalMilliseconds = 400;
	private const int PollTimeoutMilliseconds = 4000;
	...
	public static string Run(GameWindow game, string label, int potionCode, int quantity) {
		...
		if (!stampRead || nativeStamp != ExpectedNativeBuildStamp) {
			...
			text.AppendLine("Tiến trình game còn giữ DLL cũ (chưa có lệnh 327). Phải tắt client này rồi mở lại để nạp DLL mới.");
		...
		if (!game.AutoFsTransport.TrySendQuickBuyForDebug(game.Handle, potionCode, quantity, out string sendError)) {
		...
		if (increases.Count > 0) {
			...
			text.AppendLine($"KẾT QUẢ | KHÔNG thấy món nào tăng sau {elapsedMilliseconds} ms. Có thể: đã chạm giới hạn 1000 bình/ngày, không đủ tiền, "
				+ "hoặc server từ chối gói (ba byte cuối của hàm mua Auto truyền 0 — ý nghĩa chưa biết).");
		}
		return text.ToString();
	}

	private static Dictionary<string, int> Diff(Dictionary<string, int> before, Dictionary<string, int> after) {
		Dictionary<string, int> increases = [];
